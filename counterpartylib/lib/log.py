...
def log (db, command, category, bindings):
    ...
    if command == 'update':
        ...
        if category == 'credits':
            ...
            log_message = 'RPS: {} opens game with {} possible moves and a wager of {}'.format(bindings['source'], bindings['possible_moves'], output(bindings['wager'], 'SEP'))
            logger.info(log_message)

        elif category == 'rps_matches':
            log_message = 'RPS Match: {} is playing a {}-moves game with {} with a wager of {} ({}) [{}]'.format(bindings['tx0_address'], bindings['possible_moves'], bindings['tx1_address'], output(bindings['wager'], 'SEP'), bindings['id'], bindings['status'])
            logger.info(log_message)
            ...
            if bindings['status'] == 'valid':
                ...
                log_message = 'RPS Resolved: {} is playing {} on a {}-moves game with {} with a wager of {} ({}) [{}]'.format(rps_match['tx0_address'], bindings['move'], rps_match['possible_moves'], rps_match['tx1_address'], output(rps_match['wager'], 'SEP'), rps_match['id'], rps_match['status'])
            else:
